    ...
    public static class HelpContent
    {
        ...
        private static readonly Dictionary<string, ToolHelp> _toolHelp = new()
        {
            ...
            ["FeatureMatchTool"] = new ToolHelp
            {
                ...
                Parameters = new Dictionary<string, string>
                {
                    ["RetrainOriginMode"] = "기존 모델을 [Train Selected] 로 다시 학습할 때 원점(학습 중심·기준 각도·기준 이미지) 처리.\n• UseCurrentImage(기본): 원점을 현재 ROI 의 중심·각도로 다시 계산하고 기준 이미지 PNG 도 현재 장면으로 교체합니다. 부품을 옮겨 놓고 그 위치를 새 기준으로 삼을 때.\n• KeepReference: 원 학습의 원점·각도·기준 이미지를 그대로 두고 템플릿 특징만 갱신합니다. 다른 장면으로 특징을 보강하되 Match Align 마스터 포즈는 유지할 때. ROI 크기를 바꾸면 템플릿 중심 정의가 달라져 미세 오프셋이 생길 수 있습니다.\n[Add Model] 로 만드는 새 모델은 항상 현재 이미지 기준입니다. 재학습 후 연결된 Match Align 이 학습 기준을 쓰면 상태줄에 원점 갱신/유지가 표시되고, 수동 기준을 쓰면 [현재 매칭을 기준으로 등록] 재등록 안내 창이 뜹니다.",
                    ["ReferenceImagePath"] = "기준(원본) 이미지 — [Train] 성공 시 학습에 쓴 전체 장면 이미지가 레시피 폴더 옆 RefImages/에 PNG로 자동 저장됩니다.\n레시피에는 ROI로 잘라낸 템플릿만 남기 때문에, 나중에 ROI를 조정하거나 재학습하려면 [기준 이미지 불러오기] 버튼으로 그 장면을 메인 화면에 다시 띄운 뒤 작업하세요.\n재학습하면 같은 파일을 덮어써 최신 학습 장면이 유지됩니다.",
                    ["TrainMask"] = "학습 마스크 (don't-care 영역) — VisionPro의 학습 마스크 페인팅에 해당.\n[Edit Mask...]로 선택 모델의 템플릿 위에 제외 영역을 칠합니다 (모델별 별도 저장):\n• 브러시: 본체 윤곽에 붙는 반사·가변 각인을 정밀하게\n• 사각형: 그림자·배경 무늬처럼 넓은 영역을 빠르게\n• 지우개: 잘못 칠한 부분 되돌리기\n저장 시 즉시 재학습되어 특징 이미지의 반투명 빨간 영역으로 확인할 수 있습니다 — 빨간 영역에 초록 점(모델 에지)이 없어야 정상.\n편집기의 '엣지 미리보기'(기본 켬)가 학습과 같은 조건의 에지를 겹쳐 보여줍니다 — 초록=학습에 남는 에지, 주황=마스크로 제외되는 에지. 객체 윤곽의 초록 선이 주황으로 바뀌면 경계까지 과칠된 것이니 해제(UnMask) 브러시로 윤곽을 되돌리세요 (객체-그림자 경계는 객체 자신의 실루엣이라 남기는 것이 매칭에 유리).\n마스크는 여유 있게 크게 잡는 것이 안전합니다 — 에지가 남으면 객체 이동/회전 시 중심이 끌려갑니다.\n학습 ROI 크기를 바꾸고 재학습하면 크기가 안 맞는 마스크는 적용되지 않으니 다시 칠해주세요. [Clear Mask]로 해제.",
                    ["안정특징정제"] = "[안정 특징 정제 (샘플 이미지)...] — 같은 대상이 찍힌 현장 이미지 여러 장(2장 이상 권장)을 선택하면, 각 이미지에서 패턴을 찾아 학습 에지 하나하나가 실제로 일관되게 나타나는지 검증합니다.\n이미지마다 흔들리는 에지(그림자 외곽·정반사·가변 각인)는 자동으로 학습 마스크에 추가되어 재학습됩니다 — 수작업 마스킹 없이 안정적인 특징만 남습니다.\n객체 자신의 윤곽(그림자와 맞닿은 경계 포함)은 객체와 함께 움직여 매 이미지 일치하므로 유지됩니다.\n결과는 기존 마스크와 합쳐지며 [Edit Mask...]에서 확인/수정할 수 있습니다. 조명·구도가 실제 운전 조건과 같은 샘플을 쓰세요.",
                    ...
                    ["MaxModelPoints"] = "학습 시 사용할 최대 모델 에지 포인트 수. 클수록 정확하지만 속도가 느려집니다.\n• 권장: 100~500 (큰 템플릿·긴 윤곽일수록 크게)\n• Auto Tune 사용 시 검출된 에지 수 기준으로 자동 설정됩니다 (에지 8px당 1점, 100~500 범위).\n특징 이미지의 초록 점이 윤곽을 따라 듬성듬성하면 이 값을 올려 재학습하세요.",
                    ...
                    ["MaxInstances"] = "찾을 최대 인스턴스 수.\n• 1: 단일 매칭 (기존 동작)\n• 2 이상: 같은 이미지에 학습 객체가 여러 개 있을 때 점수 상위 N개를 모두 찾습니다 (트레이 부품 계수 등).\n찾은 개수는 결과의 MatchCount, 각 인스턴스는 Match0_·Match1_… 키로 출력됩니다.\n대표(최고 점수) 인스턴스는 기존 Score/CenterX/CenterY 키에 그대로 실려 Fixture·Align 연결과 호환됩니다.",
                    ["MinCoverage"] = "최소 커버리지 판정 (0~1, 0=비활성). 커버리지 = 학습 특징점 중 실제 이미지와 일치한 비율.\n점수(Score)는 모델 절반만 맞아도 ~0.5가 나와, 중심이 어긋난 '반쪽 매칭'이 통과할 수 있습니다 — 특히 학습 특징점이 적을 때.\n커버리지는 반쪽 매칭에서 ~0.5에 묶이므로 이 판정으로 기각할 수 있습니다.\n• 0.6~0.75: 권장 (부분 가림 허용 정도에 따라 조절)\n• 부품이 자주 일부 가려지는 공정이면 낮게 설정하세요.",
                    ["NmsDistanceFactor"] = "인스턴스 분리 거리 = 템플릿 짧은 변 × 이 값. 이보다 가까운 두 매칭은 같은 인스턴스로 보고 낮은 점수를 억제합니다.\n• 0.3: 좁게 (겹친 부품도 분리)\n• 0.5: 기본\n• 1.0 이상: 템플릿 크기만큼 떨어져야 별개 인스턴스",
                    ...
                    ["NumLevels"] = "이미지 피라미드 레벨 수 (1~5). 투표(위치 탐색)는 최상위(가장 축소된) 레벨에서 수행됩니다.\n• 클수록 빠르지만 위치 양자화 오차가 커집니다\n• 작은 이미지(≤100px)는 2, 일반 3, 큰 이미지 4 권장 (Auto Tune이 자동 설정)",
            ...
            ["MatchAlignTool"] = new ToolHelp
            {
                ...
                Usage = "카메라 1대로 부품 전체(또는 대표 영역)를 보는 표준 2D 얼라인 구성입니다.\n" +
                        "1) Feature Match 에서 기준 부품의 패턴을 학습합니다 (학습 중심 = 기준 포즈, 기준 각도 = 학습 ROI 각도 — 축 정렬이면 0°).\n" +
                ...
                Parameters = new Dictionary<string, string>
                {
                    ...
                    ["UseTrainedReference"] = "기준 포즈 선택.\n• 켬(기본): Feature Match 학습 시점의 패턴 중심을 기준(X_ref, Y_ref)으로, 기준 각도는 학습 ROI 각도(TrainedAngle, 축 정렬 ROI 면 0°)로 사용합니다 — 별도 등록 없이 학습만 하면 동작합니다. Feature Match 를 재학습하면 기준도 함께 바뀌거나(현재 이미지 기준) 유지되므로(기준 유지) 재학습 원점 설정을 확인하세요. TwoPoint 모드에서는 두 소스의 학습 중심이 각각 1번/2번 기준점이 됩니다.\n• 끔: 아래 Ref 값(수동 입력 또는 [현재 매칭을 기준으로 등록]으로 캡처)을 기준으로 사용합니다.",
            ...
            ["DetectionTool"] = new ToolHelp
            {
                ...
                Parameters = new Dictionary<string, string>
                {
                    ...
                    // ── 클래스별 임계값 ──
                    ["UsePerClassThresholds"] =
                        "클래스마다 다른 신뢰도 임계값을 씁니다. 켜면 클래스 목록이 나타나 각각 값을 정할 수 있습니다.\n\n" +
                        "잘 잡히는 결함과 잘 안 잡히는 결함이 섞여 있을 때 씁니다 — 예를 들어 크랙은 0.3 으로 " +
                        "느슨하게, 스크래치는 0.6 으로 엄격하게. 끄면 위의 공통 임계값 하나를 모든 클래스에 씁니다.",

                    // ── CLAHE (추론 전 대비 보정) ──
                    ["UseClahe"] =
                        "추론 전에 **국소 대비를 끌어올립니다**. 조명이 고르지 않아 어두운 쪽 결함이 묻히는 현장에서 검출률이 올라갑니다.\n\n" +
                        "이미지 전체를 밝히는 것이 아니라 구역별로 대비를 조절하므로, 밝은 곳은 타지 않고 어두운 곳만 살아납니다.\n" +
                        "조명이 이미 균일하면 켤 필요가 없습니다 — 노이즈만 늘어납니다.",
                    ["ClaheClipLimit"] =
                        "대비를 얼마나 세게 올릴지입니다. **값을 올리면 대비와 함께 노이즈도 증폭**됩니다.\n\n" +
                        "• 2.0 (기본) — 대부분의 경우\n" +
                        "• 3~4 — 많이 어두운 이미지\n" +
                        "• 6 이상 — 거친 표면에서는 노이즈가 결함처럼 보일 수 있습니다",
                    ["ClaheTileGridSize"] =
                        "이미지를 몇 × 몇 구역으로 나눠 각각 대비를 조절할지입니다 (8 이면 8×8).\n\n" +
                        "• 작게 (4) — 넓은 범위로 부드럽게\n" +
                        "• 8 (기본) — 일반적인 출발점\n" +
                        "• 크게 (16) — 좁은 범위까지 살리지만 구역 경계가 드러날 수 있습니다",

                    // ── SAHI (타일 분할 추론) ──
                    ["UseSahi"] =
                        "큰 이미지를 **여러 조각으로 잘라 각각 추론**한 뒤 결과를 합칩니다.\n\n" +
                        "고해상도 이미지에서 아주 작은 결함을 찾을 때 씁니다. 모델은 입력을 정해진 크기로 줄여서 보는데, " +
                        "그 과정에서 작은 결함이 사라지기 때문입니다. 조각마다 원본 해상도로 보므로 작은 것도 살아남습니다.\n\n" +
                        "**대신 조각 수만큼 느려집니다** — 검사 택트에 여유가 있는지 먼저 확인하세요.",
                    ["SahiTileSize"] =
                        "잘라낼 조각 한 변의 크기(px)입니다. **모델 입력 크기의 1~2배**를 권합니다(입력이 640 이면 640~1280).\n\n" +
                        "작게 하면 작은 결함에 유리하지만 조각 수가 늘어 느려지고, 크게 하면 빠르지만 축소 효과가 커집니다.",
                    ["SahiOverlapRatio"] =
                        "조각끼리 겹치는 비율입니다 (0.2 = 20% 겹침).\n\n" +
                        "**경계에 걸친 결함이 잘려 사라지는 것을 막습니다.** 결함 크기가 조각의 20% 를 넘는다면 " +
                        "겹침을 더 늘리세요. 겹침이 클수록 조각 수가 늘어 느려집니다 (0~0.5 범위).",

                    // ── Dot 분석 (박스 내부 점 군락) ──
                    ["UseDotAnalysis"] =
                        "검출된 박스 **안쪽의 점(dot) 군락**을 한 번 더 분석합니다. 점의 개수 · 배치 · 정렬 각도를 잽니다.\n\n" +
                        "인쇄 도트, 타공 패턴, 납땜 볼처럼 \"박스 안에 점이 몇 개 어떻게 박혔는지\" 가 판정 기준인 경우에 씁니다.\n" +
                        "끄면 박스 위치와 클래스만 나옵니다.",
                    ["MinDotArea"] =
                        "이 넓이(px²)보다 작은 덩어리는 점으로 보지 않고 버립니다. **노이즈를 거르는 하한**입니다.\n\n" +
                        "화면에서 점 하나의 면적을 재 보고 그보다 넉넉히 작게 잡으세요 (기본 10).",
                    ["MaxDotArea"] =
                        "이 넓이(px²)보다 큰 덩어리는 점으로 보지 않습니다. 배경 얼룩이나 붙어버린 점 덩어리를 거릅니다 (기본 500).\n\n" +
                        "점이 서로 붙어 하나로 잡히면 이 값을 넘겨 사라지므로, 검출 수가 모자라면 아래 " +
                        "**최소 점 간격**과 함께 살펴보세요.",
                    ["DotCircularityThreshold"] =
                        "얼마나 동그래야 점으로 볼지입니다 (0~1, 1이 완벽한 원). 기본 0.7.\n\n" +
                        "길쭉한 흠집이나 긁힘을 점으로 오인하지 않게 해 줍니다. 점이 타원으로 찍히는 각도라면 값을 낮추세요.",
                    ["MinDotDistance"] =
                        "이 거리(px)보다 가까운 점 두 개는 **하나로 합칩니다**.\n\n" +
                        "한 점이 두 개로 쪼개져 잡히는 경우를 막습니다. 실제 점 간격보다 작게 잡아야 합니다 — " +
                        "크게 잡으면 서로 다른 점이 합쳐져 개수가 모자라게 나옵니다 (기본 10).",
                    ["DotClaheClipLimit"] =
                        "점이 배경과 거의 같은 밝기여서 잘 안 보일 때, 박스 안쪽만 대비를 올립니다. 값이 클수록 세게 (기본 3.0).\n\n" +
                        "전처리 방식을 CLAHE 로 골랐을 때만 쓰입니다.",
                    ["DotMorphKernelSize"] =
                        "밝은(또는 어두운) 작은 점만 남기고 배경을 걷어내는 필터의 크기(px, 홀수)입니다.\n\n" +
                        "**점보다는 크고 주변 구조물보다는 작게** 잡아야 효과가 납니다 (기본 15). " +
                        "너무 작으면 점까지 지워지고, 너무 크면 배경이 남습니다. 전처리 방식을 Top-hat 계열로 골랐을 때만 쓰입니다.",
                    ["DotAdaptiveBlockSize"] =
                        "밝기 경계를 정할 때 주변 몇 px 를 보고 판단할지입니다 (홀수, 기본 25).\n\n" +
                        "• 크게 — 부드럽게 판단, 넓은 밝기 변화에 강함\n" +
                        "• 작게 — 민감하게 판단, 작은 점을 잘 잡지만 노이즈도 늘어남\n\n" +
                        "이진화 방식을 Adaptive 로 골랐을 때만 쓰입니다.",
                    ["DotAdaptiveC"] =
                        "주변 평균 밝기에서 이만큼 뺀 값을 경계로 씁니다. **값을 올리면 더 뚜렷한 것만 점으로 인정**합니다 (기본 5).\n\n" +
                        "배경 얼룩이 점으로 잡히면 올리고, 흐린 점이 안 잡히면 내립니다. " +
                        "이진화 방식을 Adaptive 로 골랐을 때만 쓰입니다."
                }
            },
            ...
            ["AnomalyTool"] = new ToolHelp
            {
                ...
                Parameters = new Dictionary<string, string>
                {
                    ...
                    ["CalibrationSigma"] =
                        "정상 샘플 폴더로 임계값을 자동 계산할 때, **정상 점수 평균에서 표준편차 몇 배 위**를 " +
                        "임계값으로 삼을지입니다 (임계값 = 평균 + 이 값 × 표준편차).\n\n" +
                        "• 2 — 느슨함. 정상인데 이상으로 잡는 일(과검)이 줄지만 약한 결함을 놓칠 수 있습니다\n" +
                        "• 3 (기본) — 일반적인 출발점\n" +
                        "• 4~5 — 엄격함. 확실한 이상만 걸립니다\n\n" +
                        "계산된 값은 제안일 뿐이며, 실제로 쓰려면 Anomaly Threshold 에 반영해야 합니다. " +
                        "정상 샘플에 결함품이 섞여 있으면 임계값이 통째로 느슨해지니, 폴더 내용을 먼저 확인하세요."
                }
            },
            ...
            ["ImageRectifyTool"] = new ToolHelp
            {
                ...
                Usage = "측정 파이프라인 맨 앞에 두어, 뒤따르는 도구들이 왜곡 없는 이미지를 보게 합니다.\n\n" +
                        ...
                        "• Checkerboard · Circles Grid 로 캘리브레이션 → Undistort 만 동작\n" +
                ...
                Parameters = new Dictionary<string, string>
                {
                    ["Undistort"] = "렌즈 왜곡(배럴·핀쿠션)을 폅니다. 직선이어야 할 제품 모서리가 화면 가장자리에서 휘어 보일 때 켭니다.\n\n" +
                                    "• 필요한 것: **Checkerboard 또는 Circles Grid** 방식 캘리브레이션 (카메라 내부 파라미터 + 왜곡 계수)\n" +
            ...
            ["PhotometricStereoTool"] = new ToolHelp
            {
                Name = "Photometric Stereo (조명 방향별 촬영 합성)",
                Description = "같은 자리에서 **조명 방향만 바꿔 찍은 사진 여러 장**을 합쳐, 표면의 미세한 " +
                              "요철을 드러냅니다.\n\n" +
                              "색이나 밝기 차이가 거의 없어 일반 촬영으로는 안 보이는 결함 — 눌린 자국, 얕은 긁힘, " +
                              "각인 누락, 표면 굴곡 — 을 찾는 데 씁니다. 조명 방향이 바뀌면 요철의 그림자도 " +
                              "바뀐다는 성질을 이용합니다.",
                Usage = "조명 3개 이상(보통 4개)을 제품 둘레에 배치하고, 하나씩 켜 가며 같은 장면을 찍습니다.\n" +
                        "각 조명의 **방향(Lx, Ly, Lz)** 과 그때 찍은 사진을 아래 목록에 등록하세요.\n\n" +
                        "조명 방향이 실제 배치와 다르면 결과가 통째로 어긋납니다 — 방향값은 대략이라도 실제와 맞춰야 합니다.",
                Parameters = new Dictionary<string, string>
                {
                    ["OutputType"] =
                        "무엇을 출력할지 고릅니다.\n\n" +
                        "• **DefectEnhanced (기본)** — 표면 굴곡을 강조한 이미지. 결함 검사용으로 이걸 쓰면 됩니다. " +
                        "뒤에 Blob 이나 Threshold 도구를 붙여 판정합니다.\n\n" +
                        "• **NormalMap** — 각 픽셀이 어느 쪽을 향하는지를 색으로 표현한 이미지. " +
                        "주로 확인·디버깅용입니다.\n\n" +
                        "• **Albedo** — 조명 영향을 걷어낸 \"제품 본래의 밝기\". 형상 대신 **인쇄·오염** 을 " +
                        "볼 때 유용합니다.",
                    ["CurvatureGain"] =
                        "DefectEnhanced 출력에서 **굴곡을 얼마나 세게 강조**할지입니다 (기본 8).\n\n" +
                        "• 값을 올리면 얕은 결함도 드러나지만 표면 거칠기까지 같이 도드라집니다\n" +
                        "• 배경이 지저분하게 일어나면 내리고, 결함이 흐리면 올립니다\n\n" +
                        "다른 출력 방식에서는 쓰이지 않습니다.",
                    ["ShadowThreshold"] =
                        "이 밝기(0~255) **이하로 어두운 픽셀**은 그 조명에서 그림자에 들어간 것으로 보고 " +
                        "계산에서 신뢰도를 낮춥니다 (기본 10).\n\n" +
                        "깊은 그림자는 표면 방향 정보를 담고 있지 않아 그대로 쓰면 결과가 튑니다. " +
                        "전체적으로 어두운 조명 조건이면 값을 낮추세요.",
                    ["HighlightThreshold"] =
                        "이 밝기(0~255) **이상으로 밝은 픽셀**은 정반사(번들거림)로 포화된 것으로 보고 " +
                        "신뢰도를 낮춥니다 (기본 245).\n\n" +
                        "광택 있는 금속면에서 특히 중요합니다 — 포화된 픽셀은 실제 밝기를 잃어버려 " +
                        "표면 방향을 잘못 계산하게 만듭니다. 번들거림이 심하면 값을 조금 낮추세요."
                }
            },
            ...
            ["CalibrationManager"] = new ToolHelp
            {
                ...
                Parameters = new Dictionary<string, string>
                {
                    ["CalibrationMode"] =
                        ...
                        "• Checkerboard — 체커보드(격자무늬) 사진으로 렌즈 왜곡까지 알아냅니다. " +
                        "Image Rectify 의 Undistort 를 쓰려면 이 방식 또는 아래 Circles Grid 여야 합니다.\n\n" +
                        "• Circles Grid (dots) — 원형(도트) 타겟으로 같은 결과를 냅니다. 산업용 타겟이 " +
                        "대부분 이 형태이고, **초점이 약간 흐려도 중심을 잘 잡아** 현장에서 더 잘 맞습니다.\n\n" +
                    ...
                    // ── Circles Grid (도트 타겟) ──
                    ["CirclePatternCols"] =
                        "한 행에 있는 **원의 개수**입니다. 체커보드와 달리 \"교차점\" 이 아니라 원을 그대로 셉니다.\n\n" +
                        "⚠ **엇갈린(비대칭) 배열**이면 위아래 두 행을 합쳐 한 행으로 셉니다 — " +
                        "OpenCV 관례입니다. 한 행에 4개, 그 아래 엇갈린 행에 5개라면 여기 넣을 값은 **9** 가 아니라 " +
                        "보통 **4** 입니다(타겟 사양서의 표기를 따르고, 안 되면 4 와 9 를 바꿔 보세요).",
                    ["CirclePatternRows"] =
                        "**원이 놓인 행의 개수**입니다.\n\n" +
                        "엇갈린 배열이면 엇갈린 행까지 각각 한 행으로 셉니다. " +
                        "행·열을 바꿔 넣어도 찾지 못하므로, 안 되면 두 값을 서로 바꿔 보세요.",
                    ["CircleSpacingMm"] =
                        "**원 중심에서 이웃 원 중심까지의 거리(mm)** 입니다. 타겟에 보통 적혀 있습니다 " +
                        "(예: `CGB-020 5x4-20mm` 이면 20).\n\n" +
                        "⚠ 엇갈린(비대칭) 배열에서는 **엇갈린 이웃 행까지의 가로 방향 거리**를 뜻합니다 — " +
                        "같은 행에서 옆 원까지의 거리는 이 값의 2배가 됩니다. 어느 쪽인지 타겟 사양서로 확인하세요. " +
                        "이 값이 틀리면 왜곡 보정은 맞게 되지만 **mm 환산만 통째로 어긋납니다** — " +
                        "결과의 Pixel Size 로 알고 있는 치수를 재 보면 바로 드러납니다.",
                    ["AsymmetricCircles"] =
                        "원의 행이 **서로 반 칸씩 엇갈려** 있으면 켭니다(벌집 모양). 격자처럼 가지런하면 끕니다.\n\n" +
                        "산업용 도트 타겟은 대부분 엇갈린 배열이라 **기본이 켜짐**입니다. " +
                        "배열 종류가 틀리면 개수가 맞아도 패턴을 찾지 못합니다 — 못 찾으면 이것부터 바꿔 보세요.",
                    ...
                    ["ReprojectionError"] =
                        ...
                        "• Checkerboard · Circles Grid — 단위는 픽셀. 보통 **1 픽셀 미만**이면 양호, 2 를 넘으면 " +
                    ...
                    ["ApplyToRecipe"] =
                        "계산 결과를 실제로 쓰이는 자리에 넣습니다. 한 번 누르면 **세 가지가 함께** 일어납니다.\n\n" +
                        "① **레시피에 저장** — 측정 도구가 mm 로 판정하고, Image Rectify 가 왜곡·기울어짐을 " +
                        "바로잡을 수 있게 됩니다\n" +
                        "② **고른 스텝의 Resolution(mm/px) 갱신** — 위 목록에서 체크한 스텝만 바뀝니다\n" +
                        "③ **레시피 파일 저장** — 따로 [Save] 를 누를 필요가 없습니다\n\n" +
                        "적용하면 VisionSetup 왼쪽 **Steps 표의 Resolution** 값이 바로 바뀌는 것으로 확인할 수 있습니다. " +
                        "값이 그대로라면 그 스텝이 목록에서 체크되지 않았다는 뜻입니다.\n\n" +
                        "레시피마다 따로 보관되므로, 카메라나 렌즈·작동거리가 다른 레시피는 각각 캘리브레이션해야 합니다.",
                    ["AppliedSummary"] =
                        "**지금 이 레시피에 저장돼 있는** 캘리브레이션입니다 — 방금 계산한 값이 아니라 저장된 값입니다.\n\n" +
                        "방식 · 픽셀 크기(mm/px) · 재투영 오차 · 캘리브레이션한 시각을 보여 주므로, " +
                        "[Apply to Current Recipe] 를 누르기 전과 후를 비교하면 적용이 됐는지 바로 알 수 있습니다.\n\n" +
                        "\"없음\" 이면 이 레시피는 아직 캘리브레이션되지 않았습니다.",
                    ["ApplyTargets"] =
                        "캘리브레이션 결과(mm/px)를 **어느 스텝의 Resolution 에 넣을지** 고릅니다. " +
                        "체크한 스텝만 바뀌고, 오른쪽에 `현재값 → 적용값` 이 미리 보입니다.\n\n" +
                        "왜 고르게 하나요? 캘리브레이션은 레시피에 한 벌만 저장되지만 Resolution 은 스텝마다 따로 " +
                        "있습니다. 한 레시피에 **카메라가 여러 대**면 렌즈·작동거리가 달라 같은 mm/px 를 모두에게 " +
                        "밀어 넣으면 다른 카메라의 측정이 틀어집니다.\n\n" +
                        "• 카메라로 직접 촬영해 캘리브레이션했다면 **그 카메라의 스텝만** 자동으로 체크됩니다\n" +
                        "• 파일이나 VMS 프레임으로 했다면 어느 카메라인지 알 수 없어 **전체가 체크**됩니다 — " +
                        "카메라가 여러 대인 레시피라면 직접 확인하고 해제하세요\n" +
                        "• 전부 해제하면 캘리브레이션만 저장되고 스텝 Resolution 은 건드리지 않습니다",
                    ["ClearRecipeCalibration"] =
                        "현재 레시피에서 캘리브레이션을 지웁니다. 지운 뒤 Image Rectify 는 이미지를 그대로 통과시킵니다.\n\n" +
                        "⚠ **스텝의 Resolution(mm/px) 값은 지워지지 않습니다** — 측정 도구는 그 값으로 계속 mm 환산을 " +
                        "합니다. 완전히 픽셀 단위로 돌리려면 Steps 표에서 Resolution 도 0 으로 바꾸세요.\n\n" +

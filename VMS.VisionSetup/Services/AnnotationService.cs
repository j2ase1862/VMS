...
namespace VMS.VisionSetup.Services
{
    ...
    {
        private static readonly Lazy<AnnotationService> _instance = new(() => new AnnotationService());
        public static AnnotationService Instance => _instance.Value;
        ...
        {
            _datasetFolderPath = Path.Combine(
                CameraService.Instance.GetAppDataFolderPath(), "Datasets");
        ...
        public AnnotationImage? AddImage(AnnotationDataset dataset, string sourceImagePath)
        {
            try
            {
                ...
                {
                    ImagePath = Path.GetFileName(destPath), // 상대 경로
                    ImageWidth = width,
                    ImageHeight = height,
                    CreatedAt = DateTime.Now,
                    ModifiedAt = DateTime.Now
        ...
        public bool ExportPaddleOcrDet(AnnotationDataset dataset, string outputPath)
        {
            try
            {
                ...
                foreach (var image in dataset.Images.Where(i => i.IsLabeled))
                {
                    ...
                    // PaddleOCR det 포맷: image_path \t [{points, transcription}]
            ...
            try
            {
                ...
                foreach (var image in dataset.Images.Where(i => i.IsLabeled))
                {
                    ...
                    foreach (var label in image.Labels.Where(l => !string.IsNullOrEmpty(l.Transcription)))
                    {
                        ...
                        // PaddleOCR rec 포맷: image_path \t label
        ...
        private string GetDatasetDirectory(AnnotationDataset dataset)
        {
            // 이름 기반 폴더 (특수문자 제거)
        ...
        private string GetImageFullPath(AnnotationDataset dataset, AnnotationImage image)
        {

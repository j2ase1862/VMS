...
| (PR 미정) | **레시피/스텝/도구 복사 3종 (VisionSetup)** — ① Recipe Manager 에 [Duplicate] 버튼: 선택 레시피를 스텝·도구·학습 데이터까지 통째로 복사해 "이름 - 복사본"으로 생성 (Web 원장에는 새 이름으로 별도 등록) ② Steps 패널에 스텝 복제 버튼(보라 복사 아이콘): 카메라/조명 설정 + 도구 파이프라인(연결 포함)을 복사해 원본 바로 뒤에 삽입 ③ 워크스페이스 도구 탭 우클릭 → [Duplicate Tool]: 파라미터·ROI·학습 데이터까지 복사한 사본 생성 (도구 간 연결은 미복사). Recipe Manager·Steps 패널·도구 우클릭 메뉴 스크린샷 갱신 | VisionSetup 매뉴얼 레시피·스텝·도구 절 |

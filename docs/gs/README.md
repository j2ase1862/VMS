...
| PR | 매뉴얼에 들어갈 내용 | 대상 절 |
|----|--------------------|--------|
| (반영 완료 2026-09-18) 별책 | **AI 학습 도구 별책을 본편 v3.0 기준으로 정정** (§5→§6.11 등 상호 참조 7곳, 표지 v1.0→v1.3, YOLO-seg→YOLOv8-seg) + **그림이 파일 안에 담긴 배포본 `docs/gs/VMS_AI학습도구매뉴얼.html` 신설** (생성기 `build_ai_tools_manual.py` — 상대 경로 그림이 파일 단독 열람 시 전부 깨지던 문제) | 별책 전체 · 본편 §10 (반영 완료) — 본편 docx/PDF 재생성 완료 |

...
**반영 완료 기준선: SW v1.13.0 (v1.5.13~v1.13.0 누적분 전체 + Web v1.5.0) — 2026-08-24 본문 반영 + §8.4 이력 + docx 재생성 완료**

| PR | 매뉴얼에 들어갈 내용 | 대상 절 |
|----|--------------------|--------|
| (잔여) | 3D 점군 뷰어 조작 스크린샷 갱신 (#275 밀도 버튼·툴바) — **실점군 로드 상태에서 수동 캡처 필요** (자동 캡처 도구는 점군 화면 미포함). 본문 서술은 반영 완료 | §3.9 (점군 화면 조작) |
| (잔여) | #354 인스톨러 브랜딩 — **설치 절차 스크린샷 교체 필요** (라이선스 화면 제거·브랜드 배너). 본문 서술은 반영 완료 (2026-08-24) | §2.1(설치), §6.5(업데이트) |
| (잔여) | 멀티 Lot·이력 스크린샷 — #356 헤더 Lot 콤보(FHD), Web #82/#84 생산 이력 날짜 피커, Web #85 WO/LOT 필터 바. 본문 서술은 반영 완료 (2026-08-21) | §4.6(Production History), §4.11(Lots) |
| (잔여) | 2026-08-24 신설 화면 스크린샷 — §3.10 Sequence Editor(에지 모드·무부하 테스트·IO Board Monitor), Image Save Settings 창(Web 연동 카드), 사이드 패널 Updates 섹션, 하단 상태바 IO n/m 칩. 본문 서술은 반영 완료 | §3.2.8, §3.3, §3.10, §6.5 |

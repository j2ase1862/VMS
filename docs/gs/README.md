...
| #400 | **레시피/스텝/도구 복사 3종 (VisionSetup)** — ① Recipe Manager 에 [Duplicate] 버튼: 선택 레시피를 스텝·도구·학습 데이터까지 통째로 복사해 "이름 - 복사본"으로 생성 (Web 원장에는 새 이름으로 별도 등록) ② Steps 패널에 스텝 복제 버튼(보라 복사 아이콘): 카메라/조명 설정 + 도구 파이프라인(연결 포함)을 복사해 원본 바로 뒤에 삽입 ③ 워크스페이스 도구 탭 우클릭 → [Duplicate Tool]: 파라미터·ROI·학습 데이터까지 복사한 사본 생성 (도구 간 연결은 미복사) ④ [Rename] 버튼: 선택 레시피 이름 변경 — 파일명은 그대로, Web 연동 레시피는 Web 원장 이름이 함께 바뀌지 않음을 안내. Recipe Manager·Steps 패널·도구 우클릭 메뉴 스크린샷 갱신 | VisionSetup 매뉴얼 레시피·스텝·도구 절 |
| Web #94 | **Web 검사항목 화면 레시피 복사·이름 변경** — 툴바에 [레시피 복사](검사 기준값 파라미터까지 복사, "이름 - 복사본") + [이름 변경](이름 중복 시 안내, VisionServer 연동 모드는 미지원 안내) 버튼 추가 (Admin 전용). 검사항목(§4.13 부근) 툴바 스크린샷·절차 갱신 | Web 매뉴얼 검사항목 절 |
| #402 | **Feature Match 다중 인스턴스 + 커버리지 판정** — ① Max Instances(1~50): 같은 이미지의 동일 객체를 여러 개 검출(결과 MatchCount·Match0_… 키, 오버레이 #번호 표기), ② Min Coverage(0~1): 특징점 일치 비율 판정으로 "반쪽 매칭(중심 이탈)" 기각 — 특징점이 적을 때 권장 0.6~0.75, ③ NMS Distance(전문가): 인스턴스 분리 거리, ④ 학습 특징점 60개 미만 시 상태바 경고, ⑤ 넓은 스케일 범위(±0.15 초과)·각도 오프 인식률 개선(내부). 설정 화면 Basic 스크린샷 재캡처 + 파라미터 표 3종 추가 | VisionSetup 매뉴얼 Feature Match 절 |
| (이번 PR) | **Feature Match Auto Tune 특징점 수 규칙 개선** — Max Model Points 자동값이 엣지 밀도가 아닌 **검출 엣지 수 기준**(8px당 1점, 100~500)으로 산정. 큰 템플릿 + 가는 윤곽에서 100점으로 고정되던 문제 해소 — 기존 레시피는 재학습(Train) 시에만 새 값 적용. 도움말(Max Model Points) 문구 갱신 | VisionSetup 매뉴얼 Feature Match 절 |

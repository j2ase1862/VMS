...
**반영 완료 기준선: SW v1.42.0 / Web v1.10.3 / MLOps v0.1.1 — 2026-09-23 v1.42.0 반영(저장 점군 .vpc 에 카메라 내부 파라미터 수록 → 저장본으로도 XY 치수 mm, 본문 서술 변경 없음 · §13.4 대상 SW, 상세 §36) · 직전 SW v1.41.1 / Web v1.10.3 / MLOps v0.1.1 — 2026-09-22 v1.41.1 반영(원형 그리드 검출이 조명에 흔들리던 문제 → §12.4 조치 한 줄 · §13.4 대상 SW 1.41.1, 상세 §35) · 2026-09-22 v1.41.0 반영(기준 좌표(Fixture) 소스 실패 시 뒤 도구 미실행 → §1.4·§5.5·§7.3·§12.4 · §13.4 대상 SW 1.39.2→1.41.0, docx/PDF 재생성 완료 — 198쪽 · 그림 154장 유지, 상세 manual_change_history.md §34) · 2026-09-18 GS 제출본 문구 정리 반영(§6.11 세그 도구 라이선스 고지 제거 · §7.3/§7.8 타사 제품명 제거, docx/PDF 재생성 완료 — 198쪽 · 그림 154장 유지) · 2026-09-17 v1.40.1 (#510 캘리브레이션 창 거절 시 프레임 미수신 fix — §5.11 서술과 동작 일치, 매뉴얼 변경 없음)** (직전 기준선 SW v1.40.0 / Web v1.10.3 / MLOps v0.1.1 — 2026-09-17 v1.40.0 반영(#505 얼라인 템플릿 통합 → §5 템플릿 표·§7.8 · #506 팔레트 Alignment 카테고리 → §5 왼쪽 패널 표·§6.4 신설(§6.5~§6.15 번호 이동) · #507 메인 화면 그림 재캡처 · #504 WeldTeach 메뉴 삭제(§5.1, v3.0 에 선반영). docx/PDF 재생성 완료 2026-09-17 (#512 — VMS_사용자매뉴얼_v2.0.docx 19.6MB · PDF 198쪽 · 그림 154장, §6.4 Alignment 81쪽 확인))** (직전 기준선 SW v1.39.2 / Web v1.10.3 / MLOps v0.1.1 — 2026-09-17 매뉴얼 v3.0 전면 개편(13장 구성: 개요→설치→설정 마법사→VMS 운전→VisionSetup→비전 도구 40종→검사 설정 예제→Web→MLOps→AI 학습 도구→유지보수→문제 조치→부록, 그림 154장, 변경 이력 절 폐지 — 아래 표의 #495~#500 항목 반영 완료)** (직전 기준선 SW v1.37.3 / Web v1.10.2 — 2026-09-15 반영(§7.1 로그인 유지 운용 기준·§7.2 화면 공통 동작·§7.4 안돈 보드 여는 방법·§7.7 생산 시작 후 제품·라인 잠금·§7.10 키오스크 안내/세션 종료·§7.13 불량 코드 잠금·§7.15 감사 로그 야간 조회·§7.16 PDF 보고서/파레토 기간·§10.1 업데이트 후 Web 접속 불가 신설·§10.3 안돈 로그인 화면 신설·§10.3 포트 5292 정정·§10.4 연결 신호 거부·§11.4 v1.37.1~v1.37.3 + Web v1.10.1·v1.10.2 · §8.5 라인 번호 대역(다중 서버) · §5.11 카메라 캘리브레이션 신설 · §10.2 캘리브레이션 트러블슈팅 3건 · docx 재생성)** (직전 기준선 SW v1.37.0 / Web v1.10.0 — 2026-09-14 반영(§4.2.10 MLOps 비활성 칩 신설·§4.3 MLOps 카드(원본·비활성 사유)·§7.6 검사 사진 열람 권한·§8.3 업로드 큐(검사 시각·보관함·사진 자동 축소)·§8.5 라인 번호 0~99·§9.4 백업 동작·§10.1 트러블슈팅 4건 신설·§11.4 v1.35~v1.37 + Web v1.10.0 · 별책 §2 웹에서 학습하기·학습 결과 파일 · 설정 마법사 표 3행(Client Index·MLOps 주소·API Key) · 신규 캡처 2장(`vms_ctl/status_mlops_issue_chip.png`·`vms_ctl/sec_system_log_wo_warning.png`, ControlCapturer 장면 S4·S5 추가) · docx/PDF 재생성)**) (직전 기준선 SW v1.33.0 / Web v1.9.0 — 2026-09-10 §9~§12 반영(#429 D-FINE·#437 레지스트리 참조·#439/#440 등록·내려받기·#441/#442 RF-DETR-seg·#446 MLOps 수집·#427/#445/#447 수정, 본편 §3 고급 설정·§4.3·§5.10 신설·§11.4 + 별책 §1·§2.1·§3·§4 v1.1 + 스크린샷 21_dlg_imagesave·51_rfdetr_seg_panel·AppSetup 2단계 MLOps 컨트롤) (직전 기준선 v1.29.0 / 2026-09-04 Match Align 원점·각도(#424) + Web 로그인 유지(Web #96)) (직전 기준선 v1.28.1 / 2026-09-04 단독 모드 로컬 생산이력(#417) 반영) (직전 기준선 v1.24.0 / 2026-09-01 매뉴얼 v2.0 전면 개편 (앱별 11장 구성: 개요→설치→AppSetup→VMS 운전→VisionSetup→DeepLearning→Web→워크플로→관리자→트러블슈팅(5절 확장)→부록. 산출물 `VMS_사용자매뉴얼_v2.0.docx`. 생성기: 캡처는 '첫 문단 뒤' 삽입 규칙 + 삽입/누락 검증 로그 + 마법사 §3.2·관리자 다이얼로그 §4.9 앵커 이동 — ⚠ 앵커 = 4장/5장 h2 제목 문자열, 제목 변경 시 gen_user_manual.js 동기화 필수) (§3.2.9 예측 칩·§3.9 Feature Match 학습/다중 인스턴스·복사 3종·얼라인 템플릿·재연결 창·§4.9 Web 복사·§8.4 이력 반영. ⚠ gen_user_manual.js 의 WIZARD_BEFORE 앵커가 HTML §2.6 제목 문자열 — 제목 변경 시 앵커도 함께))
(직전 기준선 v1.13.0 의 스크린샷 안내는 이력에서 유지 — 데모 캡처 절차 web_capture_partial.js/seed_demo.js, 운영 DB 무접촉)

| PR | 매뉴얼에 들어갈 내용 | 대상 절 |
|----|--------------------|--------|
| v1.42.4 점군 내부 파라미터 · 3D Geometry | **VisionSetup 에서 저장한 점군에도 카메라 내부 파라미터가 실린다**(종전에는 VMS 에서 넘겨받는 과정에서 빠져 VisionSetup 저장본은 X/Y 치수를 mm 로 잴 수 없었다) · **치수 환산(AutoFromCamera)을 켜도 3D Geometry 점-평면 높이가 맞는다**(종전 90mm 가 55mm). 상세 §40 | §12 한 줄 검토 |
| v1.42.3 현장 3D 검증 결함 3건 | **Registration 이 평평한 부품에서 발산하던 결함 수정**(Registration 쓰던 레시피는 결과가 달라지며 새 값이 맞다) · **측정 실패 화소가 3D 점으로 쓰여 PlaneFit 평면도 ~1,900mm·3D Geometry 가 카메라 거리를 성공으로 내던 결함 수정** · **PlaneFit 을 다른 도구와 함께 두면 Run 이 중단되던 결함 수정**. 3D Geometry 안내 문구(수동 점 켜짐이면 연결한 점 무시). 상세 §39 | §6 Registration·PlaneFit·3D Geometry · §12 |
| v1.42.2 WO 완료 사이클 경계 정지 | **WO 가 완료되면 진행 중인 제품까지 판정·출력한 뒤 자동 운전이 멈춘다** — 종전에는 완료 응답이 오는 순간 즉시 멈춰, 그때 설비에 들어와 있던 제품이 판정 없이 NG 처리되고 화면·이력에도 남지 않았다. 수동 [정지] 는 종전대로 즉시. 상세 §38 | §4 AUTO RUN 작업지시 완료 |
| v1.42.1 시퀀스 IO 보드 테스트 · 연결 보정 | **[▶ 테스트 출력] · [▶ 테스트 읽기] 가 IO 보드 노드에서 실제로 보드 채널을 쓰고 읽는다** — 매뉴얼 §5 서술("IO 보드 채널에 기록 … 테스트 출력으로 바로 써 봅니다")대로 동작하게 됨(종전에는 노드 장치를 보지 않고 늘 PLC 로 가 보드 출력이 나가지 않았다). 모니터 패널의 **보드 출력 채널 현재값 = 편집기가 마지막으로 쓴 값**(출력은 되읽을 수 없음). ADLink 출력 쓰기 실패가 이제 오류로 드러난다(AUTO RUN 은 해당 노드에서 시퀀스 오류). **화면의 연결선과 실행 경로가 어긋난 시퀀스를 불러오기·운전 시작 때 맞춘다**(종전에는 선이 보여도 트리거마다 검사 없이 처음으로 돌아갔다) — 편집기 상태줄 "연결 N건을 화면의 선과 맞춤", 운전 로그 `시퀀스 연결 보정/문제`. 새 Next 선은 기존 선을 대체. 상세 §37 | §5 시퀀스 편집기 표(서술 변경 없음, 모니터 현재값 한 줄 추가 검토) · §12 트러블슈팅 |
| (반영 완료 2026-09-23) v1.42.0 점군 내부 파라미터 | **저장한 .vpc 로도 XY 치수가 mm 로 나온다** — 종전에는 저장본에서 치수 자동 환산이 꺼져 배율을 따로 적어 둬야 했다. 조작 절차 변화 없음(형식 확장) | §13.4 (반영 완료) — docx/PDF 재생성 완료 |
| (반영 완료 2026-09-22) v1.41.1 원형 그리드 검출 | **조명·노출이 달라져도 원형 타겟이 검출되도록** 단계적 검출로 변경 (종전 26조건 중 13건 → 25건). 실패 안내에 노출·조명 확인 추가 | §12.4 · §13.4 (반영 완료) — docx/PDF 재생성 완료 |
| (반영 완료 2026-09-22) v1.41.0 기준 좌표 게이트 | **Fixture 소스가 실패하면 뒤 도구를 실행하지 않고 NG** — 종전에는 기준을 못 받아도 뒤 도구가 직전 사이클 ROI 로 측정해, 제품이 없어도 값이 나오고 공차에 들면 AUTO RUN 이 OK 를 냈다. 사유 문구 2종(`기준 위치를 주는 도구가 실패하여 건너뜀` · `기준 좌표를 받지 못해 건너뜀`). **기존 레시피는 판정이 달라질 수 있고 새 판정이 맞다** | §1.4 · §5.5 · §7.3 · §12.4 · §13.4 (반영 완료) — docx/PDF 재생성 완료 |
| (반영 완료 2026-09-18) 별책 | **AI 학습 도구 별책을 본편 v3.0 기준으로 정정** (§5→§6.11 등 상호 참조 7곳, 표지 v1.0→v1.3, YOLO-seg→YOLOv8-seg) + **그림이 파일 안에 담긴 배포본 `docs/gs/VMS_AI학습도구매뉴얼.html` 신설** (생성기 `build_ai_tools_manual.py`) + **별책 PDF `docs/gs/VMS_AI학습도구매뉴얼.pdf`(13쪽) 신설**(Chrome 헤드리스 생성기 `html2pdf_chrome.ps1` · 인쇄용 흰 바탕 CSS) | 별책 전체 · 본편 §10 (반영 완료) — 본편 docx/PDF 재생성 완료 |
| (반영 완료 2026-09-18) 문서 정합성 | **제품설명서 팔레트 표 12 → 15 카테고리 39종** (Alignment·3D 분리·Surface Analysis 누락, 그림도 6월 캡처 → 9/17 재캡처로 교체) + **§4.2 런타임 동봉 반영·Windows 11** + **매뉴얼 §1 도구 수 40 → 39종**(#504 WeldTeach 제거 반영, §5·§6 과 불일치였음) | 제품설명서 §1.1·§6.3.4·§7.2 · 매뉴얼 §1 (반영 완료) — docx/PDF 재생성 완료 |
| (반영 완료 2026-09-18, 같은 PR) 타사 제품명 | **예제 장의 TMED 비유 제거** — §7.3 "TMED 방식의 '위치 보정(Fixture)' 에 해당합니다" → 용어 소개 문장으로 교체, §7.8 "TMED 의 '로봇 얼라인' 처럼" → 괄호 용어로 교체. 타사 제품명을 설명 기준으로 쓰지 않는다(하드웨어 호환 목록·데이터셋 형식 이름은 사실 기재라 유지) | §7.3 · §7.8 (반영 완료) — docx/PDF 재생성 완료(2026-09-18) |
| (반영 완료 2026-09-18, 같은 PR) 라이선스 문구 | **세그 도구 설명에서 라이선스 고지 제거** — §6.11 YOLOv8-seg 설명의 "Ultralytics 는 AGPL-3.0 이므로 상용 배포 라이선스를 확인" 문장과 RF-DETR-seg 의 대비 문장을 뺐다(제품에 Ultralytics 코드 없음 · 학습 스택은 AiTools Feature 로 인증 빌드에서 제외 · 제3자 라이선스 정본은 배포 정책 §2.6). 도구 물음표 도움말·패널 하단 안내도 같은 문구로 맞춤 | §6.11 (반영 완료) — docx/PDF 재생성 완료(2026-09-18) |
| (기준선 v1.40.1 포함 2026-09-17, 매뉴얼 변경 불필요) #510 | **캘리브레이션 창 [Grab from VMS] 가 거절됐는데도 프레임을 받던 결함** — VMS 가 운전(AUTO RUN)·라이브 중이라 촬영을 거절하면 예전에는 공유 메모리에 남은 운전 사이클 사진을 그대로 받아 거절 사유가 "Received frame" 에 덮였다(v1.40.0 현장 E1). 이제 거절·무응답·오류면 프레임을 읽지 않고 사유만 남긴다. 매뉴얼 §5.11 "운전 · 라이브 중엔 거절" 서술과 동작이 일치하게 됨 | §5.11 (변경 없음) · v1.39.2 체크리스트 B4 가 이제 실제로 통과 |
| (반영 완료 2026-09-17, 같은 PR) #506 | **툴 팔레트에 Alignment 카테고리 신설** — Pattern Matching 아래 있던 Match Align · Multi-Step Align 을 예제 템플릿 갤러리 '얼라인' 탭과 같은 이름의 카테고리로 옮김(팔레트 15 카테고리). 매뉴얼 6장은 §6.4 Alignment 신설로 §6.5~§6.15 번호 한 칸 이동 | §5 왼쪽 패널 표 · §6.4 (반영 완료) — `vs/full_main_scene.png` 재캡처 완료(2026-09-17, Alignment 카테고리·WeldTeach 메뉴 제거 반영, 2048×1232) |
| (반영 완료 2026-09-17, 같은 PR) #505 | **예제 템플릿 '얼라인' 탭의 2D 항목을 Match Align 1개로 통합** — '2점 매치 얼라인' 템플릿은 Match Align 의 Mode 를 TwoPoint 로 바꾼 것과 같아 갤러리에서 뺐다(툴과 템플릿이 겹쳐 혼동). 2D 템플릿 제목이 툴명으로 시작한다("Match Align — 2D 매치 얼라인", "Multi-Step Align — 2-스텝 얼라인"). 2점이 필요하면 Feature Match 를 하나 더 붙이고 Mode 를 TwoPoint 로 | §5 템플릿 표 · §7.8 얼라인 예제 (반영 완료) — 갤러리 스크린샷 `vs/dlg_TemplateGallery.png` 은 3D 탭이 열린 그림이라 재캡처 불필요 |

...
| #358~#361 (+Web #87~#89) | **SW 라이선스 — 본문 반영 완료 (2026-08-25)**: §2.6 설치·활성화 신설 + §2.4 체크리스트 갱신 + §4.18 좌석 현황 신설 + §4.2 메뉴 표 + §8.4 이력 + 마법사 7단계 "Security & License" 갱신(gen_user_manual.js — WIZARD_BEFORE 앵커가 §2.6 제목이 됨, 문구 변경 주의) + 스크린샷(10_appsetup_step7 교체 · 62_web_SW_라이선스 신규 · appsetup_ctl/P7 갱신) + Admin 매뉴얼 재생성. ⏳ **docx 재생성만 잔여** — 작업 시점에 Word 가 파일을 잠가 실패. `cd docs/gs/pipeline && node gen_user_manual.js` 재실행 (parse_manual.py 는 이미 실행됨) | 완료 (docx 잔여) |

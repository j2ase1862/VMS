...
| PR | 매뉴얼에 들어갈 내용 | 대상 절 |
|----|--------------------|--------|
| (반영 완료 2026-09-17, 같은 PR) #506 | **툴 팔레트에 Alignment 카테고리 신설** — Pattern Matching 아래 있던 Match Align · Multi-Step Align 을 예제 템플릿 갤러리 '얼라인' 탭과 같은 이름의 카테고리로 옮김(팔레트 15 카테고리). 매뉴얼 6장은 §6.4 Alignment 신설로 §6.5~§6.15 번호 한 칸 이동 | §5 왼쪽 패널 표 · §6.4 (반영 완료) — ⏳ **`vs/full_main_scene.png` 재캡처 대기**(팔레트 트리가 옛 구성 + WeldTeach 메뉴 잔존) |

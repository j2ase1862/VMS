...
| #417 | **단독 모드 로컬 생산이력 1·2단계** — ① Recent Inspections 패널이 단독 모드에서도 기록(NG 코드 자리에 실패 도구 이름, 로컬 레시피명, 도움말 문구) ② 검사 결과 로컬 SQLite 영구 저장(`inspection_history.db`, 기본 90일) + Retention Settings 전역 행·체크박스·미리보기·프리셋 InspHistory 열 + 백업 포함. 상세: manual_change_history.md §3·§4 | §3.5 단독 모드 (+§4 Recent Inspections·§4.9 Retention Settings·백업/복원·§11.4 이력) |

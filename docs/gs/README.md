...
| (대기) | **Web 연동 파라미터 배지** — 툴 설정에서 ParamCode 링크가 걸린 파라미터의 콤보 아래에 "Web 연동 중 (#코드: 값) — 운전 값은 Web에서 변경" 배지 상시 표시. 로컬 입력값과 Web 값이 다르면 **주황 경고**("로컬 x ≠ Web y — 운전은 Web 값을 사용")로 승격 + 툴팁에 상세 안내. 매뉴얼 §4 공통 절에 "연동 파라미터 수정 위치" 설명 추가 필요 | §4(비전 툴 공통), §6(Web 연동) |
| (대기) | **Web 수동 완료 → VMS 즉시 반영** (Web #73 과 세트) — Web 화면에서 작업지시를 완료 처리하면 VMS 가 즉시 AUTO RUN 자동 정지 + 완료 다이얼로그 + WO 선택 해제 (계획 수량 자동 완료와 동일 동작). SignalR 미연결이어도 다음 업로드 응답으로 동일 처리(폴백). 완료된 WO 로 업로드된 검사는 수량 미집계 경고가 시스템 로그에 남음 | §3.8(작업지시), §5(시퀀스 운전), Web 매뉴얼 WO 절 |
| #354 | **인스톨러 UI 브랜딩 + 업데이트 진행률 창** — MSI 설치 마법사에 VASIM 네이비/오렌지 브랜딩(환영 화면·배너) 적용, 라이선스 동의 화면 제거(설치 단계 1개 감소), 인앱 업데이트 시 msiexec 기본 진행률 바 대신 다크 테마 진행률 창(버전·현재 작업·% 표시). 설치 절차 스크린샷 교체 필요 | §2(설치), §7(업데이트) |
...
| #356 | **헤더 Lot 콤보 상시 노출** — FHD(1920) 화면에서도 Ctx 영역의 Lot 콤보가 항상 표시(기존엔 컴팩트 헤더에서 숨겨져 멀티 Lot 선택 불가 — 2026-08-21 현장). WO 수동 입력칸은 컴팩트에서 계속 숨김. 멀티 Lot 절 스크린샷 FHD 기준 확인 필요 | §3.8(작업지시/Lot) |
| Web #83 | **작업지시 페이지 실시간 갱신** — VMS 운전 중 사이클마다 생산/양품/NG·진척률이 새로고침 없이 자동 반영, 계획 도달 시 상태 칩 즉시 "완료됨" 전환. "수량이 안 바뀌면 F5" 류 안내가 매뉴얼에 있으면 제거 | Web 매뉴얼 WO 절 |
| Web #82+#84 | **생산 이력 하루치 조회 정상화** — 시작=종료 같은 날짜 선택 가능(날짜 피커 2개로 교체) + 요약 카드 소실 수정 + 날짜 필터 UTC 정리(감사 로그·품질 분석·유지보수·작업자 페이지 포함, KST 9시간 밀림 해소) + Excel 검사 시각 로컬 표기 | Web 매뉴얼 생산 이력 절 |
| Web #85 | **생산 이력 WO/LOT 필터** — 작업지시 콤보 + LOT 콤보(선택 WO 의 발행 Lot)로 이력 필터링, 테이블 "WO / LOT" 열·Excel 내보내기에 Work Order/Lot 열 추가. 구형 이력(미귀속)은 필터 시 제외됨 명시 | Web 매뉴얼 생산 이력 절 |
...
| (대기) | **NG 이미지 Web 표시 정상화** (Web #74 와 세트) — 이미지 전달 모드 Auto 가 "같은 머신이면 무전송"이던 동작을 폐지하고 **항상 업로드** (같은 머신 구성에서 NG 상세에 이미지가 영원히 비던 문제). AUTO RUN 사이클의 모든 검사가 상관 키를 공유해 이미지↔이력 매칭 안정화, NG 이력에는 NG 이미지 우선. **설정 주의: "NG 이미지 Web 전송" 토글을 켜야 전송** (로컬 저장 체크와 별개). SharedPath 모드는 미구현 참조라 선택 지양 문구 추가 | §3.4(이미지 저장 설정), §6(Web 연동), §7 트러블슈팅 |
...
| (대기) | **멀티 Lot 병행 운용 + 실시간 Lot 콤보** (Web #80 과 세트) — 헤더 Lot 입력칸이 **Open Lot 콤보**로 변경: WO 선택 시 Open Lot 목록 자동 조회, Web에서 Lot 발행/마감 시 실시간 갱신(60초 폴링 안전망). 1개면 자동 선택(기존 경험 유지), 여러 개 병행이면 작업자가 콤보에서 선택 — 선택된 Lot으로 검사 결과 귀속. 선택 Lot이 마감되면 자동 재선택/비움 + 시스템 로그 안내. **Lot ID 수동 타이핑은 콤보 선택으로 대체**됨 | §3.8(작업지시/Lot), §5(시퀀스 운전), Web 매뉴얼 Lot 절 |
| (대기) | **작업지시 운전 결과 업로드 정상화 + 미연동 경고** (#350) — 레시피를 먼저 로드한 채 WO를 선택하면 파라미터 연동이 빠져 사이클 업로드가 통째로 생략되던 문제(완료 다이얼로그·진행율·안돈·NG 이미지 침묵) 수정. 어느 경로로 와도 연동 자동 일치 + Web 미연동 상태로 WO 운전 시작 시 시스템 로그 경고와 상태바 "Running — 경고: Web 미연동" 표시. 트러블슈팅에 "WO 수량이 안 오를 때 = 상태바/시스템 로그 확인" 항목 추가 | §3.8(작업지시), §5(시퀀스 운전), §7 트러블슈팅 |
| (대기) | **VisionSetup 실행 직후 Web 연동 콤보 정상화** (#348) + 레시피 로드 시 상태바 Web 파라미터 진단(#345) — 사용자 절차 변화 없음, §6 트러블슈팅 갱신 후보 | §6(Web 연동), §7 트러블슈팅 |
| (대기) | **패턴 매칭 각도·스케일 판정 + Web 연동** — FeatureMatch/ShapeMatch 툴 설정에 Pose Judgment 섹션 신설: 찾은 매칭(다중 인스턴스는 1순위)의 각도/스케일이 허용 하한~상한을 벗어나면 NG (기본 비활성 — 기존 레시피 영향 없음). 4개 한계값 모두 Web 파라미터 연동 콤보 지원 — **Web 패턴 프리셋의 Angle/Scale Lower·Upper Limit 항목과 1:1 대응** (Position X/Y는 여전히 미지원). §4 두 툴 판정 항목에 추가 필요 | §4(비전 툴: FeatureMatch·ShapeMatch), §6(Web 연동) |
| (대기) | **ParamCode 콤보 레시피 추종 수정** — 툴 설정의 Web 연동 콤보가 "처음 로드된 레시피" 고정이던 것을 현재 편집 중인 레시피의 파라미터로 자동 전환 (레시피 전환 시 즉시 반영). 사용자 절차 변화 없음 — §6 "연동 파라미터가 안 보일 때 = 레시피 확인" 트러블슈팅 후보 | §6(Web 연동), §7 트러블슈팅 |
| (대기) | **인앱 업데이트 (다운로드→설치→재시작 자동화)** — 새 버전 다이얼로그에서 "예" 선택 시 브라우저 이동 없이 앱 안에서 MSI 다운로드(사이드 패널 진행률·취소 버튼, SHA-256 무결성 검증) → 관리자 승인(UAC) 1회 → 자동 설치 후 VMS 자동 재실행. **Web 서비스가 켜져 있던 PC 는 업데이트 후 자동으로 다시 시작됨** (종전 AppSetup 수동 시작 불필요). 자동 운전 중에는 업데이트 불가. 실패 시 기존 브라우저 수동 다운로드로 안내. 설치 로그: ProgramData\BODA\VMS\update-bootstrap.log | §3.2(업데이트 확인), §6(Web 연동), §7 트러블슈팅 |

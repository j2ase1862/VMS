...
| #358 | **라이선스 1단계(호환 모드)** — 기동 시 라이선스 상태 검사가 추가되었으나 현 단계는 경고만(차단·UI 변화 없음, 감사로그 LicenseCheck 기록). 매뉴얼 반영은 AppSetup 활성화 UI(지문 표시·파일 가져오기)가 나오는 2단계에 몰아서 — 그때 설치/활성화 절차 절 신설 | (2단계에서) 설치·초기 구성 절 |

...
## 35. 원형 그리드 검출이 조명에 흔들리던 문제 (v1.41.1) — **반영 완료 (docx/PDF 재생성 2026-09-22)**

| 항목 | 내용 |
|------|------|
| 날짜 | 2026-09-22 |
| 대상 | 본편 §12.4 트러블슈팅 · §13.4 문서 정보 |
| 변경 종류 | 조치 안내 한 줄 + 대상 SW 버전 |

### 무엇이 문제였나 (사내 기록)

`SimpleBlobDetector` 가 밝기를 **50~220 구간에서 10 씩** 훑어 이진화하는 OpenCV 기본값을 그대로 쓰고 있었다.
조명이 어두워지거나 과노출이면 원·배경 밝기가 그 구간 밖으로 밀려나 blob 이 몇 개씩 빠지고,
원 하나만 빠져도 `FindCirclesGrid` 가 격자를 세우지 못한다. **원의 모양·크기 판별(면적·원형도 필터)이나
캘리브레이션 계산의 문제가 아니었다.**

실촬영 타겟(`D:\original_20260915_174900.png`)을 밝기만 바꿔 26 조건으로 돌린 결과:

| 방식 | 성공 | 1회 소요 |
|------|------|---------|
| 기본값 50~220 step10 (종전) | 13/26 | 318ms |
| 10~250 step10 | 20/26 | 423ms |
| 10~250 step5 | 24/26 | 853ms |
| 백분위 적응형 | 11/26 | 350ms (오히려 나빠짐) |
| **단계적 시도 (채택)** | **25/27** | 정상 조명 ~300ms · 실패 시 0.8~1.7초 |

검출 좌표는 어느 단계에서 찾아도 같았다(중심 간격 355.6~358.6px) — 단계는 "찾느냐" 만 가르고 측정값을 바꾸지 않는다.

### 매뉴얼 반영

| 절 | 내용 |
|----|------|
| §12.4 레시피 편집·판정 | "캘리브레이션에서 패턴을 찾지 못했다" 항목에 **노출·조명 확인** 한 줄 추가 |
| §13.4 문서 정보 | 대상 소프트웨어 `VMS 1.41.0` → **`VMS 1.41.1`** |

제품 화면의 실패 안내에도 같은 취지를 넣었다(행·열 확인 + 노출·조명 확인). 관련: [[project_calibration_circles_grid]]

---

## 36. 저장 점군(.vpc)에 카메라 내부 파라미터 수록 (v1.42.0) — **반영 완료 (docx/PDF 재생성 2026-09-23)**

| 항목 | 내용 |
|------|------|
| 날짜 | 2026-09-23 |
| 대상 | 본편 §13.4 문서 정보 (대상 SW) — 본문 서술 변경 없음 |
| 변경 종류 | 파일 형식 확장 (사용자 조작 절차 변화 없음) |

### 무엇이 문제였나 (사내 기록)

grab 점군은 X/Y 가 뎁스맵 화소, Z 만 mm 다. 화소→mm 환산에 쓰는 fx/fy 를 `PointCloudData.SaveToFile`
이 저장하지 않아, 저장본을 다시 불러오면 `PointCloudCluster` 의 AutoFromCamera 가 꺼지고 수동 배율로
폴백했다("⚠ Auto scale unavailable"). **저장한 점군만으로는 XY 치수를 mm 로 잴 수 없었다** —
GS 3D 시험을 저장 점군으로 대체하려면 배율값을 별도 문서로 제출해야 하는 제약이었다
([[project_3d_vpc_validation]] §3-4).

### 형식 확장 방식 — 후행 블록

색상 뒤에 `VPCX` + 버전 + fx/fy/cx/cy(double×4) 를 **선택적으로** 덧붙인다. 헤더(VPC1)는 그대로 둔다.

- 구버전 로더는 색상까지만 읽고 EOF 를 확인하지 않으므로 **새 파일도 그대로 읽는다**(점군만).
- 새 로더는 블록이 없는 **옛 파일도 그대로 읽는다**.
- 내부 파라미터가 없으면 블록을 안 붙여 **바이트가 옛 형식과 동일**하다.
- 꼬리가 잘린 파일은 내부 파라미터만 포기하고 점군은 살린다(로드 실패로 만들지 않는다).

회귀 테스트 5건(`PointCloudVpcIntrinsicsTests`) — 왕복·옛 파일·바이트 동일·잘린 꼬리 +
**저장→로드 후 AutoFromCamera 가 실제로 mm 치수를 내는지**(핵심). 수정 전 코드에서 3건 실패 확인.

### 매뉴얼 반영

§13.4 대상 소프트웨어 `VMS 1.41.1` → **`VMS 1.42.0`**. 화면 조작 절차가 바뀌지 않아 본문 서술은 그대로다
(Scale Mode = Auto From Camera 설명은 §6 도구 레퍼런스에 이미 있다).

---

## 37. 시퀀스 IO 보드 테스트 버튼 · 화면 연결선과 실행 경로 불일치 (v1.42.1) — **반영 대기**

| 항목 | 내용 |
|------|------|
| 날짜 | 2026-09-23 |
| 대상 | 본편 §5 시퀀스 편집기 노드 표(Input Check · Output Action) — 서술은 이미 올바름 |
| 변경 종류 | 결함 수정 (매뉴얼 서술과 동작 일치) |

### 무엇이 문제였나 (사내 기록)

실증 PC 에서 IO 보드(PCI-7432) 로 OK/NG 출력을 구성하던 중 확인. 노드 속성 패널의
**[▶ 테스트 출력] · [▶ 테스트 읽기]** 가 노드의 장치(DeviceId)를 보지 않고 늘 메인 PLC 로 갔다.

- PLC 가 없으면 "PLC가 연결되어 있지 않습니다" 로 끝나 보드 출력이 나가지 않았다.
- PLC 가 연결돼 있으면 보드 채널 번호("0")를 **PLC 주소로 해석해 PLC 에 썼다**.
- 무부하 테스트 실행도 보드가 연결되지 않은 보드 노드를 PLC 경로로 흘려보냈다.
- 모니터 패널의 보드 **출력** 행은 입력(DI)을 읽어, 같은 번호의 입력 값(예: 트리거)이 출력 값처럼 보였다.
- ADLink `DO_WriteLine`/`DO_WritePort` 실패는 Debug 로그에만 남아 Release 에서 보이지 않았다.

### 수정

- 테스트 버튼·무부하 실행: 보드 노드는 보드로만 간다(미연결이면 사유 표시, PLC 로 새지 않음).
- 모니터 보드 출력 행: 편집기가 마지막으로 쓴 값 표시(쓴 적 없으면 `—`). 출력 되읽기(DO_ReadLine)는
  공식 헤더로 검증되지 않아 쓰지 않았다([[project_sequence_io_board]] — 공식 샘플·헤더만 신뢰).
- ADLink 출력 쓰기 실패는 예외 → 편집기는 "[테스트 출력 실패] … DO_WriteLine 실패 (에러 n)",
  **AUTO RUN 은 해당 Output Action 노드에서 시퀀스 오류**(로그 + 카메라 상태 Error). 결과 매핑 쓰기 경로는
  종전대로 삼킨다(변경 없음). Advantech 경로는 이번 범위 밖(여전히 무음).

회귀 테스트 5건(`SequenceEditorBoardTestIoTests`) — 보드 ON/OFF 쓰기(PLC 없이) · 보드 미연결 시 PLC 로 새지 않음 ·
쓰기 실패 표시 · 보드 입력 읽기.

### AUTO RUN 점검에서 함께 드러난 결함 — 화면의 선과 실행 경로 불일치

같은 실증 파일(`Process Sequence.seq.json`)에서 **Wait Trigger → Inspection 선은 화면에 있는데
Wait Trigger 의 `nextNodeId` 가 비어** 있었다. 엔진은 참조만 따라가므로 **AUTO RUN 은 트리거마다 검사 없이
시퀀스를 끝내고 처음으로 돌아갔다** — 오류도 로그도 없어 "트리거는 받는데 OK/NG 가 안 나온다" 로만 보였다.

경로: 편집기가 한 노드에서 Next 선을 여러 번 그리게 허용했고(참조는 마지막 선만), 그 선을 지우면
참조가 비어 앞의 선만 화면에 남았다.

- **편집기**: 새 Next(·Repeat) 선이 기존 선을 **대체**한다("기존 연결을 대체했습니다").
- **불러오기·운전 시작 시 보정** (`SequenceLinkReconciler`, VMS.PLC 공용): 참조가 비었거나 그려지지 않은 곳을
  가리키는데 같은 라벨의 선이 정확히 하나면 **보이는 선으로 참조를 채운다**. 참조를 지우지는 않는다(기본 시퀀스의
  Repeat → End 처럼 선 없는 참조 보존). 선이 여럿이면 고르지 않고 알린다. Start 에서 닿는데 다음 노드가 없는
  곳(End 제외)도 알린다.
  - 편집기: 상태줄에 "연결 N건을 화면의 선과 맞춤(저장하면 파일에 반영)" / 문제 요약.
  - AUTO RUN: 시작 시 로그 `시퀀스 연결 보정 — …`(경고) · `시퀀스 연결 문제 — …`(오류). 운전은 계속.

회귀 테스트: `SequenceLinkReconcilerTests` 5건(실증 파일 모양이 보정 전 검사 0회 → 보정 후 1회 · 선 2개는 보고만 ·
Branch False 채움 · 막다른 노드 보고 · 기본 시퀀스 무변경) + `SequenceEditorLinkTests` 2건(선 대체 · 불러오기 보정).

### 현장 확인 (실증 PC)

1. 편집기 모니터 패널에서 장치 연결 → Result OK ON 노드 선택 → [▶ 테스트 출력] → **DO0 실측 ON**
2. 속성에서 ON 체크 해제 → [▶ 테스트 출력] → **DO0 실측 OFF** (체크만 바꾸면 쓰지 않는다 — 버튼을 다시 눌러야 함)
3. 상태줄에 "[테스트 출력 실패] … DO_WriteLine 실패" 가 나오면 에러 코드를 기록
4. VMS AUTO RUN 시작 → 시스템 로그에 `시퀀스 연결 보정` / `시퀀스 연결 문제` 가 없는지(있으면 내용 기록) →
   트리거 1회 = 검사 1회 + OK 면 DO0, NG 면 DO1 실측

### 매뉴얼 반영

§5 Output Action 행 서술은 그대로 맞다. 모니터 패널 설명에 "보드 출력 채널은 되읽을 수 없어 편집기가
마지막으로 쓴 값을 표시" 한 줄 추가를 검토. 체크박스는 설정값이고 [▶ 테스트 출력] 을 눌러야 기록된다는 점도 함께.

---

## 38. WO 완료 시 자동 운전을 사이클 경계에서 정지 (v1.42.2) — **반영 대기**

| 항목 | 내용 |
|------|------|
| 날짜 | 2026-09-23 |
| 대상 | 본편 §4 AUTO RUN · 작업지시 완료 흐름 (서술 보강 검토) |
| 변경 종류 | 결함 수정 |

### 무엇이 문제였나 (사내 기록)

실증 PC 보고: "WO 마지막 작업에서 판정 전에 완료 다이얼로그가 뜨고, 그 제품은 무조건 NG, 화면에 이미지도 안 올라온다".

WO 완료는 N번째 사이클의 **판정 업로드 응답**으로 온다(Web `IsPlannedQuantityReached`). 업로드는 운전과 비동기라
응답이 도착할 때 엔진은 이미 다음 트리거를 기다리거나 다음 제품을 처리 중이다. 종전에는 그 순간 운전을 **즉시 취소**했다.

- 트리거 대기 중이었다면: 정지 후 들어온 제품은 아무도 응답하지 않아 촬영·판정·출력 없음 → 설비 NG/타임아웃.
- 다음 제품 검사 도중이었다면: 검사는 끝나도 Branch·OK 출력이 취소 → 설비 NG, 사이클 업로드·로컬 이력도 없음.

또한 완료 처리기가 업로드 응답 스레드에서 **동기 Invoke 로 모달 다이얼로그**를 띄워, 업로드 작업이 다이얼로그가 닫힐 때까지 묶였다.

### 수정

- `SequenceEngine.RequestStopAtCycleBoundary()` — 사이클 밖(트리거 대기)이면 대기를 끊고 바로 종료, 사이클 진행 중
  (트리거 충족 또는 검사 시작 이후)이면 판정·출력·Repeat(사이클 집계)까지 수행한 뒤 종료. 보드 대기(취소를 삼킴)·
  PLC 대기(예외) 두 경로 모두 정상 종료.
- `AutoProcessService.StopAfterCycleAsync(timeout)` — 경계 정지 요청 → 운전 루프 종료 대기 → 30초 초과 시 즉시 정지로 전환 →
  일반 정리. 정지 시 **대기 중인 사이클 업로드를 먼저 기다린 뒤**(최대 10초) 사이클 버퍼를 비운다. 수동·자동 정지가 겹쳐도 정리는 한 번.
- WO 완료 처리: 사이클 경계 정지 사용 + 다이얼로그는 비동기로 띄움. **수동 [정지] 는 종전대로 즉시.**
- 로그: `진행 중인 사이클을 마친 뒤 자동 운전을 정지합니다.` → `사이클 경계에서 자동 운전을 정지했습니다.`

회귀 테스트 3건(`SequenceEngineCycleBoundaryStopTests`) — 보드 트리거 대기 중 즉시 정지·검사 없음 / PLC 트리거 대기 중
예외 없이 정지 / 검사 도중 요청 시 OK 출력·사이클 집계 1건 후 정지(다음 트리거 안 받음).

### 남는 것 (VMS 만으로는 못 막음)

정지가 트리거 대기 중에 일어나면 그 뒤 설비가 투입한 제품(계획 초과분)은 검사되지 않는다 — 설비에 "WO 완료" 신호를
주어 투입을 멈추게 하는 연동이 필요(별건 검토).
